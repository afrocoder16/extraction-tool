...
def group_lines(words: list[dict], page_w: int) -> list[dict]:
    ...
    Grouping is geometric, not by Tesseract's line numbering. The black and red
    layers are OCR'd as two separate images, so each carries its own independent
    block/paragraph/line counters — trusting them puts a red word and the black
    word beside it on different lines, and the page comes out interleaved
    (`«በቅዱስ ገብርኤል ቃል» (ምስራች) በነገረኝ` instead of `(ምስራች) በነገረኝ «በቅዱስ ገብርኤል ቃል»`).
    Vertical overlap is layer-agnostic, so red and black rejoin on the line they
    were actually printed on, and sorting by x within the line restores the
    reading order.
    """
    ...
    for word in sorted(words, key=lambda w: w["top"]):
        ...
        for group in buckets:
            ...
            # Same line if this word's centre falls inside the run's vertical
            # span, allowing a little slack for baseline wobble in the scan.
            slack = word["height"] * 0.4
            if top - slack <= centre <= bottom + slack:
                group.append(word)
                break
        else:
            buckets.append([word])
...
def analyse_page(
    ...
    red_mask = cv2.imread(str(work / "red_mask" / name), cv2.IMREAD_GRAYSCALE)
    ...
    for layer_path, forced_red in (
        (work / "black_layer" / name, False),
        (work / "red_layer" / name, True),
    ):
...
def analyse_pages(
    pages: list[int], work: Path, tesseract: Path, tessdata: Path, lang: str,
    page_images: Path | None = None,
) -> list[dict]:
    out_dir = work / "layout"
    out_dir.mkdir(parents=True, exist_ok=True)
    results = []
    for page in pages:
        result = analyse_page(page, work, tesseract, tessdata, lang)
...
def main() -> None:
    ...
    for result in analyse_pages(pages, args.work, args.tesseract, args.tessdata, args.lang):
        print(
