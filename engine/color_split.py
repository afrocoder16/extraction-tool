...
2. Decorative red ornament borders (e.g. page 147 header).
   ...
   Known limitation: a chain ornament made of many *disconnected* links is not
   rejected, because each link passes the per-component glyph test. Grouping them
   by row was tried and reverted - a line of red text is also "many similar short
   components spanning the page", and every threshold that caught the ornaments on
   pages 19 and 147 also swallowed the red titles and body lines on those same
   pages. The cost is asymmetric: a missed ornament only inflates mask_red_ratio
   slightly on that page, making one verify metric noisier, whereas swallowing red
   text corrupts the mask that all red assignment depends on. So these are left in.
...
def ensure_dirs(out_root: Path) -> dict[str, Path]:
    """out_root is the book's work/ directory."""
    dirs = {
        name: out_root / name
