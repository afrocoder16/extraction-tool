...
Responses are cached to output/v2/transcripts/page_NNN.json; a page with a cached
...
PAGE_SCHEMA = {
    ...
    "required": ["page", "notes", "corrections", "blocks"],
    "additionalProperties": False,
}
...
def transcribe_page(
    client: anthropic.Anthropic, page: int, v2_root: Path, effort: str
) -> dict:
    name_png = f"page_{page:03d}.png"
    layout_path = v2_root / "layout" / f"page_{page:03d}.json"
    if not layout_path.exists():
        raise FileNotFoundError(f"run layout first; missing {layout_path}")
    layout = json.loads(layout_path.read_text(encoding="utf-8"))

    original_b64 = encode_image(v2_root.parent / "page_images" / name_png)
    overlay_b64 = encode_image(v2_root / "debug" / name_png)
...
def protected_pages(v2_root: Path) -> dict[int, str]:
    ...
    for backup in (v2_root / "transcripts").glob("page_*.original.json"):
...
def transcribe_pages(
    pages: list[int], v2_root: Path, effort: str, force: bool
) -> list[dict]:
    out_dir = v2_root / "transcripts"
    out_dir.mkdir(parents=True, exist_ok=True)

    # Proofreading is the expensive human input in this pipeline; never let a
    # re-run silently destroy it.
    if not force:
        protected = protected_pages(v2_root)
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        raise SystemExit(
            ...
            "before running the transcription stage."
        )
    ...
    for page in pages:
        ...
        result = transcribe_page(client, page, v2_root, effort)
...
def main() -> None:
    ...
    transcribe_pages(pages, args.v2_root, args.effort, args.force)
